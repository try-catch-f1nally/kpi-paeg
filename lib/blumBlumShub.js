...
module.exports = class BlumBlumShub {
  ...
  static generateKeyPair() {
    const p = numberToBigInteger(generateCongruent3mod4());
    const q = numberToBigInteger(generateCongruent3mod4());
    const n = p.multiply(q);
    return {publicKey: {n}, privateKey: {p: p.intValue(), q: q.intValue()}};
  }

  /**
   * @param {string} message
   * @param {PublicKey} publicKey
   * @returns {{message: string, x0: BigInteger}}
   */
  static encrypt(message, publicKey) {
    ...
    return {message: convertToText(encrypted), x0};
  }
  ...
  static decrypt(message, x0, privateKey) {
    ...
    const decrypted = binaryChars.map((char, i) => {
      ...
      const generated = x0.modPow(power, n);
