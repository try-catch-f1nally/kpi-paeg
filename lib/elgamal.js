...
/**
 * @typedef Encrypted
 * @property {BigInteger} a - a
 * @property {BigInteger[]} bArray - bArray
 */
...
module.exports = class ElGamal {
  ...
  static encrypt(data, publicKey) {
    const { p, g, y } = publicKey;
    const k = numberToBigInteger(generateRandomInt(1, p.intValue() - 2));
    const mArray = data.split('').map((char) => char.charCodeAt(0));
    const a = g.modPow(k, p);
    const bArray = mArray.map((m) => y.modPow(k, p).multiply(numberToBigInteger(m)).mod(p));
    return { a, bArray };
  }
   ...
   * @returns {string}
   */
  static decrypt(encrypted, x, p) {
    const { a, bArray } = encrypted;
    const result = bArray.map((b) => b.multiply(a.modPow(x, p).modInverse(p)).mod(p).intValue());
