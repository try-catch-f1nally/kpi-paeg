...
module.exports = class ElGamal {
  ...
  static encrypt(data, publicKey) {
    const { p, g, y } = publicKey;
    const k = numberToBigInteger(generateRandomInt(1, p.intValue() - 1));
    const a = g.modPow(k, p);
    const bArray = [...Buffer.from(data)].map((m) => y.modPow(k, p).multiply(numberToBigInteger(m)).mod(p));
    return { a, bArray };
  }
   ...
   * @returns {string}
   */
  static decrypt(encrypted, privateKey, publicKey) {
    const { p } = publicKey;
    const { x } = privateKey;
